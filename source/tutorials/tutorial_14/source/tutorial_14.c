...
static void vTaskLow( void *pvParameters )
{
    ...
    printf("Low: Trying to TAKE semaphore...\r\n");
    xSemaphoreTake(xBinarySemaphore, portMAX_DELAY);
    
    printf("Low: got it. Doing work for 250ms...\r\n");
    vTaskDelay(pdMS_TO_TICKS(250));
    
    printf("Low: My priority is still %d.\r\n", uxTaskPriorityGet(NULL));
...
static void vTaskMedium(void *pvParameters)
{
    ...
    for (;;)
    {
        printf("Medium: Running (Low is still lower priority)... %d and priority %d\r\n", count++, uxTaskPriorityGet(NULL));
        vTaskDelay(pdMS_TO_TICKS(50));
    }
}
...
int main(void)
{
    ...
    /* Initially give the semaphore so it's available */
    xSemaphoreGive(xBinarySemaphore);
